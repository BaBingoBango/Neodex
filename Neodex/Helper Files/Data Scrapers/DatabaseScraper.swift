...
/// Scapes the Pokémon Database website for the data on a certain Pokémon.
/// - Parameter number: The National Pokédex number of the Pokémon to retrieve the data for.
/// - Parameter slice: If provided, the provided HTML will be right-sliced before any processing is done.
/// - Returns: A Pokémon object containing the data retrieved. If data could not be retrieved, the value returned is nil.
func scrapePokemon(number: Int, slice: String?) -> Pokemon? {
    
    // MARK: 1: Prepare link
    let link = "https://pokemondb.net/pokedex/\(number)"
    
    // MARK: 2: Get and unwrap link contents
    var webpage = ""
    let wrappedWebpage = getURLContents(link: link)
    if wrappedWebpage == nil {
        return nil
    } else {
        webpage = wrappedWebpage!
    }
    
    // MARK: 3: Parse the webpage string for Pokémon data
    
    // MARK: 3-0: If we should, activate the slice!
    
    let preSlice = webpage
    
    if slice != nil {
        
        webpage = webpage.components(separatedBy: slice!)[2]
...
//    }
    ...
    if !(preSlice.components(separatedBy: "is a <a href=").count >= 2) {
        // The page is using "an"
        pokemonType1 = preSlice.components(separatedBy: "is an <a href=")[1].components(separatedBy: "\">")[1].components(separatedBy: "</a>")[0]
        pokemonType2 = preSlice.components(separatedBy: "is an <a href=")[1].components(separatedBy: "\">")[2].components(separatedBy: "</a>")[0]
    } else {
        // The page is using "a"
        pokemonType1 = preSlice.components(separatedBy: "is a <a href=")[1].components(separatedBy: "\">")[1].components(separatedBy: "</a>")[0]
        pokemonType2 = preSlice.components(separatedBy: "is a <a href=")[1].components(separatedBy: "\">")[2].components(separatedBy: "</a>")[0]
    }
